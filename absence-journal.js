(function(root) {
  ...
  function renderPanel() {
    ...
    panel.innerHTML=`<div class="panel-toolbar no-print"><button class="ghost-button" type="button" data-absence-action="open">Отсутствие / замещение</button>${replacements.length?'<button class="ghost-button" type="button" data-absence-action="print">Печать листа замещения</button>':''}</div>
      ...
      <section class="substitution-sheet"><h3>Лист замещения · ${escapeHtml(name(id))} · ${escapeHtml(month)}</h3><table class="journal-detail-table"><thead><tr><th>Дата</th><th>Время</th><th>За преподавателя</th><th>Группа / ученик</th><th>Предмет</th><th>Пед.</th><th>Кц</th></tr></thead><tbody>${replacements.sort((a,b)=>a.date.localeCompare(b.date)||(a.time||'').localeCompare(b.time||'')).map(r=>`<tr><td>${formatDate(r.date)}</td><td>${escapeHtml(r.time||'')}</td><td>${escapeHtml(r.sourceName)}</td><td>${escapeHtml(r.studentName)}</td><td>${escapeHtml(SchoolModel.subjectLabel(r))}</td><td>${formatNumber(SchoolModel.roundHours(r.pedHours))}</td><td>${formatNumber(SchoolModel.roundHours(r.kcHours))}</td></tr>`).join('')}</tbody><tfoot><tr><th colspan="5">Всего</th><th>${formatNumber(replacements.reduce((n,r)=>n+SchoolModel.roundHours(r.pedHours),0))}</th><th>${formatNumber(replacements.reduce((n,r)=>n+SchoolModel.roundHours(r.kcHours),0))}</th></tr></tfoot></table><p>Преподаватель ____________________</p></section>`;
  }
  ...
  function topic(id) {
    ...
    openModal('Занятие по замещению',`<form class="modal-form" data-absence-form="topic" data-record-id="${escapeAttr(id)}"><p>${formatDate(r.date)} · ${escapeHtml(r.studentName)} · ${formatNumber(SchoolModel.roundHours(r.pedHours))} Пед., ${formatNumber(SchoolModel.roundHours(r.kcHours))} Кц</p>${r.participantIds.length>=8?`<label>Тема урока<textarea name="topic" maxlength="2000">${escapeHtml(r.topic||'')}</textarea></label><button class="primary-button" type="submit">Сохранить</button>`:'<p>Для этого занятия тема не требуется. Часы заданы листом замещения.</p>'}</form>`);
  }
